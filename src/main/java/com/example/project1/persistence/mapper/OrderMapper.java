...
public interface OrderMapper {
     ...
     */
    @Insert("""
        INSERT INTO t_order (order_no, user_id, status, total_amount)
        VALUES (#{orderNo}, #{userId}, #{status}, #{totalAmount})
        """)
     ...
     */
    @Select("""
        SELECT id, order_no, user_id, status, total_amount, created_at, updated_at
        FROM t_order
        WHERE id = #{id}
        """)
